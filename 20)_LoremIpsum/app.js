...
form.addEventListener('submit', function (e) {
  e.preventDefault(); // This prevents the default behaviour of the form ..ie to submit to another page which we don't want to do here
  const value = parseInt(amount.value);
  const random = Math.floor(Math.random() * text.length);

  // Empty value, -1, Bigger than 9
  if (isNaN(value) || value < 0 || value > 9) {
    
    result.innerHTML = `<p class="result">${text[0]}</p>`; // Display 1 paragraph
  }
  else {
